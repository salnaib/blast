...
jQuery.fcbkListSelection = function(elem, width, height, row){
    ...
    var getContent = function(elem, tab){
        switch (tab) {
            case "all":
                elem.children("li").addClass("hidden").hide(); //modified by Ali 2013-02-03
				elem.children("li:not(.already_messged)").removeClass("hidden").show(); //modified by Ali 2013-02-03
				$(".htext").text("Unhide");	
				$('#filterSearchBox').val('');
				$("#view_all").addClass('rounded');
				$("#view_selected").removeClass('rounded');
                break;
                
            case "selected":
                elem.children("li:not([addedid])").hide();
                elem.children("li[addedid]:not(.already_messged)").show(); //modified by Ali 2013-02-03
				$("#view_selected").addClass('rounded');
				$("#view_all").removeClass('rounded');
                break;
    ...
    var createTabs = function(elem, width){
        ...
        '<ul class="selections" id="selections"><li id="view_all" class="view_on rounded">' +
        ...
        '<li id="view_unselected" class="hidden"><a onclick="return false;" href="#">Unselected</a></li></ul>' +
